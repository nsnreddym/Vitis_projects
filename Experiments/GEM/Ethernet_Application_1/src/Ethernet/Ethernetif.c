...
err_t EMACps_Read(struct netif *netif)
{
   u32 bufstate = 0;
   u32 bufflags;
   u32 bufadd;
...
err_t EMACps_init(struct netif *netif)
{
   EMACPS_device_struct *DevPtr;

   DevPtr = netif->state;

   DevPtr->config.DeviceId = XPAR_XEMACPS_0_DEVICE_ID;
   DevPtr->config.BaseAddress = XPAR_XEMACPS_0_BASEADDR;
   DevPtr->config.IsCacheCoherent = XPAR_XEMACPS_0_IS_CACHE_COHERENT;

   DevPtr->macaddrlen = 6;
   for (int i = 0; i < 6; i++)
   {
      DevPtr->macaddr[i] = netif->hwaddr[i];
   }

   netif->flags = NETIF_FLAG_ETHARP | NETIF_FLAG_LINK_UP;// | NETIF_FLAG_UP;
   ...
   /* Read GemVersion */
   GemVersion = ((Xil_In32(DevPtr->config.BaseAddress + 0xFC)) >> 16) & 0xFFF;

   /* Config device */
   EMACps_Config(DevPtr);

   nRxFrames = 0;
   nTxFrames = 0;

#ifdef InterruptsEnable
   DevPtr->recv_handler = EMACps_RxHandler;

   /* Setup interrupts */
   EMACps_SetupIntrSystem(&EMACPS_GIC, DevPtr, EMACPS_IRPT_INTR);
#endif


   /* Start device */
   EMACps_devStart(DevPtr);
...
u8 EthernetLinkStatus(EMACPS_device_struct *devicePtr)
{
   u16 eth_phy_statusreg;

   /* Read PHY interrupt status registers */
   EMACps_PhyRead(devicePtr, EMACps_PhyAddr, PHY_INTR_STATUS_REG, &eth_phy_statusreg);

   //printf_eth("INT STatus = %04x \r\n",eth_phy_statusreg);

   /* Read Link status */
   EMACps_PhyRead(devicePtr, EMACps_PhyAddr, PHY_LINK_REAL_REG, &eth_phy_statusreg);

   //printf_eth("PHY Link STatus = %04x \r\n",eth_phy_statusreg);


   /* Check status */
   if (IEEE_STAT_LINK_STATUS_REAL & eth_phy_statusreg)
   {
      return TRUE;
   }
   else
